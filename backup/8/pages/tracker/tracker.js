$(document).ready(function(){
	...
	//////////// making requests 
	function get_task_id_by_name ( name, project_id ) {
		var task_id = 0
		tasks.forEach( function ( task ) {
			if ( task['name'] == name && task['project_id'] == project_id ) task_id = task[ 'id' ];
		} );
		return task_id;
	}
	...
	function get_project_id_by_name ( name ) {
		...
		projects.forEach( function ( project ) {
			if ( project[ 'name' ] == name ) project_id = project[ 'id' ];
		} );
		return project_id;
	}
	
	function get_customer_id_by_name ( name ) {
		var customer_id = 0;
		customers.forEach( function ( customer ) {
			if ( customer[ 'name' ] == name ) customer_id = customer[ 'id' ];
		} );
		return customer_id;
	}
	...
	function post_new_time_entry ( task_id ) {
		...
		post(
			...
			function ( responseText ) {
				if ( JSON.parse( responseText ).message == "Time entry added successfully." ) {
	...
	function post_new_project ( name, customer_id, rate, success_callback ) {
		...
		post(
			...
			function ( responseText ) {
				...
				if ( JSON.parse( responseText ).message == "Project created successfully." ) {
					...
					get(
						...
						function ( responseText ) {
							...
							success_callback( get_project_id_by_name( name ) );
	...
	function post_new_task ( name, project_id, success_callback ) {
		post(
			...
			function ( responseText ) {
				...
				if ( JSON.parse( responseText ).message == "Task created successfully." ) {
					get(
						'https://api.zipbooks.com/v1/tasks?token=' + log_in_obj.token,
						function ( responseText ) {
							tasks = JSON.parse( responseText );
						
							success_callback( get_task_id_by_name( name, project_id ) );
	...
	function post_new_customer ( name, success_callback ) {
		post(
			...
			'&phone='+dom_phone.value+
			'&address_1='+dom_address_1.value+
			'&address_2='+dom_address_2.value+
			'&city='+dom_city.value+
			'&state='+dom_state.value+
			'&postal_code='+dom_postal_code.value+
			'&country='+dom_country.value,
			function ( responseText ) {
				
				console.log(responseText);
				
				if ( JSON.parse( responseText ).message == "Customer created successfully." ) {
					get(
						'https://api.zipbooks.com/v1/customers?token=' + log_in_obj.token,
						function ( responseText ) {
							customers = JSON.parse( responseText );
							
							success_callback( get_customer_id_by_name( name ) );
	...
	function submit_click () {
		...
		if ( dom_customer_select.value === 'new_customer' ) {
			...
			if ( !dom_email.value.match(/.+@.+\..+/) ) {
				display_error('Enter email correctly.');
				error=true;
			}
			if ( dom_phone.value === '' ) {
				display_error('Enter phone.');
				error=true;
			}
			if ( dom_address_1.value === '' ) {
				display_error('Enter address 1.');
				error=true;
			}
			if ( dom_address_2.value === '' ) {
				display_error('Enter address 2.');
				error=true;
			}
			if ( dom_city.value === '' ) {
				display_error('Enter city.');
				error=true;
			}
			if ( dom_state.value === '' ) {
				display_error('Enter state.');
				error=true;
			}
			if ( dom_postal_code.value === '' ) {
				display_error('Enter postal code.');
				error=true;
			}
			if ( dom_country.value === '' ) {
				display_error('Enter country.');
				error=true;
			}
		...
		if ( !error ) {
			...
			if ( dom_customer_select.value === 'new_customer' ) {
				post_new_customer(
					dom_customer_select.nextElementSibling.querySelector('.item[data-value="new_customer"]').innerHTML,
					function ( customer_id ) {
						post_new_project(
							dom_project_select.nextElementSibling.querySelector('.item[data-value="new_project"]').innerHTML,
							customer_id,
							dom_input_project_rate.value,
							function ( project_id ) {
								post_new_task(
									dom_task_select.nextElementSibling.querySelector('.item[data-value="new_task"]').innerHTML,
									project_id,
									function ( task_id ) {
