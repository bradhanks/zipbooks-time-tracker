	...
	( function ( document ) {
		...
		var view = {
			...
			default_subject: "[::user_first_name::] Invoice #::number::",
			default_message: "You've got a new invoice from ::user_first_name:: for ::total::.  View your invoice here: \n ::link:: \n\n Thanks, Vlas",
			
			display_message: function ( message ) {
				alert( message );
			},
			
			show_send_dialog: function ( data ) {
				
				$('#sendModal').fadeOut();
				$('#subject').val( view.default_subject.replace( '::user_first_name::', data['user_first_name'] ) );
				$('#message').val( view.default_message.replace( '::user_first_name::', data['user_first_name'] ).replace( '::total::', data['total'] ) );
				
				
			},
			
			hide_send_dialog: function () {
		...
		var observers = {
			...
			'send_invoice_click': function () {
				...
				if ( data ) {
					...
					$('#send_to').val( data['customer_email'] );
					$('#bcc_email').html( data['user_email'] );
					$('#message').val( "You've got a new invoice from "+data['user_first_name']+" "+data['user_last_name']+" for "+data['total']+". View your invoice here: \n ::link:: \n\n Thanks, \n "+data['user_first_name'] ).css({height:'initial'});
					$('#subject').val(
						'[::name::] Invoice #::name::-::number::'
						.replace( '::name::', data[ 'user_first_name' ] )
						.replace( '::name::', data[ 'user_first_name' ] )
		...
		$('#popup_send').click( function () {
				...
				if ( data ) {
					...
					data.bcc = $('#bcc')[0].checked ? '1' : '0';
					data.attach_pdf = $('#pdf')[0].checked ? '1' : '0';
